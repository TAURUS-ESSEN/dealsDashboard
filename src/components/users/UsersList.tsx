...
export const UsersList = () => {
  const { openModal, users, clients, isLoading, loadingErrors, isBackgroundFetching }: Props = useOutletContext();
  ...
  const assignedClientsCount = users.reduce(
    (count, user) => count + (clientsMapByUser.get(user.userKey)?.length ?? 0),
    0,
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700 pb-4">
        <div>
          <span className="flex gap-1 items-center">
            ...
            {isBackgroundFetching && <span className="text-xs text-slate-300">Refreshing...</span>}
          </span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] table-fixed">
            ...
            <tbody className="divide-y divide-slate-200 text-sm">
              {isLoading && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center font-semibold text-slate-700">
              ...
              {loadingErrors.users && (
                <tr>
                  <td colSpan={7}>{loadingErrors.users}</td>
                </tr>
              )}
              {!isLoading &&
                !loadingErrors.users &&
